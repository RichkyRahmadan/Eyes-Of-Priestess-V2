...
/**
 * OtpService — OTP (One-Time Password) simulation service.
 *
...
public class OtpService {
    ...
    public Uni<String> requestOmen(String phone, String purpose) {
        ...
        return RedisService.storeOmen(omenToken, otp, phone)
    ...
    public Uni<String> verifyOmen(String omenToken, String omenCode) {
        return RedisService.verifyOmen(omenToken, omenCode)
                .flatMap(phone -> {
                    if (phone == null) return Uni.createFrom().nullItem();

                    // OTP is valid — issue verification token
                    String verificationToken = UUID.randomUUID().toString();
                    return RedisService.storeVerificationToken(verificationToken, phone)
    ...
    public Uni<String> getVerifiedPhone(String verificationToken) {
        return RedisService.getVerifiedPhone(verificationToken);
    }
    ...
    public Uni<Void> consumeVerificationToken(String verificationToken) {
        return RedisService.invalidateVerificationToken(verificationToken);
    }
