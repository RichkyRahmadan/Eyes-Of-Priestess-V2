...
/**
 * SealExceptionMapper — Global JAX-RS exception mapper for Seal Sanctum.
 * Converts domain exceptions and validation errors into the standard ApiResponse format.
 */
@Provider
public class SealExceptionMapper implements ExceptionMapper<Exception> {

    private static final Logger LOG = Logger.getLogger(SealExceptionMapper.class);

    @Override
    public Response toResponse(Exception exception) {

        if (exception instanceof SealException ex) {
            return Response
                .status(ex.status)
