...
/**
 * RedisService — The Crystal (Redis) operations for the Seal Sanctum.
 *
 * Manages:
...
public class RedisService {
    ...
    public Uni<Long> incrementPinFail(String userId) {
        String key = PIN_FAIL_PREFIX + userId;
        return values().incr(key)
                .call(count -> redisDs.key(String.class).expire(key, 1800L));
    }

    public Uni<Void> clearPinFail(String userId) {
        return redisDs.key(String.class)
                .del(PIN_FAIL_PREFIX + userId)
                .replaceWithVoid();
    }
}
