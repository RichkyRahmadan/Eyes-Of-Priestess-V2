...
@Provider
public class VaultExceptionMapper implements ExceptionMapper<Exception> {

    private static final Logger LOG = Logger.getLogger(VaultExceptionMapper.class);

    @Override
    public Response toResponse(Exception exception) {
        if (exception instanceof VaultException ex) {
            return Response
                    .status(ex.status)
