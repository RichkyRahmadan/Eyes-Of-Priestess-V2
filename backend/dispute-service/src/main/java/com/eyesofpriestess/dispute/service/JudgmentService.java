...
import com.eyesofpriestess.dispute.dto.response.JudgmentCaseResponse;
import com.eyesofpriestess.dispute.entity.JudgmentCase;
import com.eyesofpriestess.dispute.event.JudgmentResolvedEvent;
import com.eyesofpriestess.dispute.exception.JudgmentException;
import com.eyesofpriestess.dispute.repository.JudgmentCaseRepository;
    ...
    private Uni<JudgmentCaseResponse> executeSettlement(JudgmentCase c, UUID buyerId, UUID sellerId, BigDecimal amount) {
        return switch (c.resolutionType) {
            case RELEASE_TO_COUNTERPART ->
                    WalletClient.releaseEscrow(buyerId, sellerId, amount, c.covenantId.toString())
                            .map(res -> {
            ...
            case REFUND_TO_INITIATOR ->
                    WalletClient.refundEscrow(buyerId, amount, c.covenantId.toString())
                            .map(res -> {
                                emitResolved(c, buyerId, sellerId, amount);
                                return JudgmentCaseResponse.from(c);
                            });
            ...
            case SPLIT -> {
                ...
                yield WalletClient.splitEscrow(buyerId, sellerId, amount, c.covenantId.toString(), sellerPct)
                        .map(res -> {
                            emitResolved(c, buyerId, sellerId, amount);
                            return JudgmentCaseResponse.from(c);
                        });
